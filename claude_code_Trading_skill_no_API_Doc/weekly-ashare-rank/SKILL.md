...
2. **WebSearch 补环境盲区**（脚本看不到的）：隔夜美股(纳指/费城半导体)、富时中国A50期货、
   人民币汇率异动、跨周末政策预期。搜索词：`"美股 费城半导体 收盘"`、`"富时中国A50期货 今日"`。
2.5. **T+1 前瞻 nowcast（展示 + 背离告警用，绝不自动改仓位/打分）**：`market_gate.py` 现已自动多输出一条
   ...
   **+ 情绪延续/退潮微调**，附**扩张窗口回测的方向命中率 / 跳空 MAE**（无前视），写进 `market_gate_latest.json` 的 `t1_forecast`。
   引擎那部分**联网不到外盘**，所以我（Claude）要做两件事：
   - **叠加外盘方向**：把第2步 WebSearch 到的隔夜美股 / 费半 SOX / 富时 A50 夜盘 / 人民币，与 nowcast 方向**人工对一下**，
...
- **（2026-07-01 新增·P10 头部风控三件套，引擎已自动执行——2026-06-30 泰格排头部却亏最多、康龙质地好反抗跌的教训）**：过去靠我手动 prose、极易漏执行的三件事现已硬编码进引擎，Agent③ **默认沿用引擎产出的顺序、不用再手动重排**：
  1. **M2 当天暴涨兑现硬剔除**：`chg1`(T日涨幅)≥7% = 多为消息/板块高潮拉起、次日易见光死回吐 → 引擎标 `cashout=True`、`entry_mode=暴涨兑现·默认剔除(观察)`、`position_pct=0` 并**沉出头部**。走样本 A/B 验证：该剔除使头部超额 +0.29%→+0.53%、多空价差 0.8→1.11%（edge 不降反升）。
  ...
  3. **排序铁律**：引擎最终 `candidates[]` 已按「①可买+基本面好 → ②可买+基本面差 → ③不可买(pos=0)」三档排好，**物理顺序=名次**。Agent③ 通常直接采用；仅当 Agent② 有≤7天新鲜独立催化等强理由才微调，且**微调后仍须保证前2只不是暴涨兑现/基本面差票**（若想补 M3 题材去重=前2强制不同 `industry`，说一声即可加）。
...
> 🔮 **T+1 前瞻(Agent⓪·展示用·不改仓位/打分)**：<偏多/偏空/中性> · P涨a/P跌b · 预计跳空<区间> · 置信度<档>(回测命中率h) · 外盘:<WebSearch一句同向/相左> · <背离告警如有>
