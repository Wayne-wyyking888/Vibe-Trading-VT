...
>
> **建议**：①默认用 **N=3**——资金周转效率最高档里统计最扎实的（出场即可重跑 skill 复用资金）；②每笔超额随 N 变大是真的，但 N=7/10 的截面重叠严重（10日仅≈4个独立窗口），数字底座薄；③**爆雷率随 N 单调升**（持有越久撞崩盘日概率越高），"拿久点更稳/能涨回来"已被否决（崩后 84-89% 先触-8%止损，真·等回来仅 9-11%）——想吃长动量用 **P14-3 赢家延展**（浮盈才延，输家快刀），不是拉长 N；④ N=5 是权重预设的低谷（既非超短配比也非波段配比），慎用。
>
