...
| top / 数量 / 几只 | 详细候选数量（不是最终报告条数） | `--top K` | 20（2026-07-10定案；引擎默认 **pool=400/top=20**——三组A/B证明粗排对综合分≈噪声(40vs200重合仅1/20、200vs400仍有9/20头部来自200名开外)，400≈初筛后全宇宙；top=20经gate剔除后最坏样本仍剩6只干净票，够出8只表；**Agent②深查预算保持前~10只不变，不随top扩大**；最终报告表与HTML只呈现头部8只） |

解析不到就用默认值。解析后**复述一遍**给用户确认口径（T、T+1、N、板块、top）。

> **📊 N(持有天数)口径速览（2026-07-14 A/B：各N独立重校准权重+走样本validate，60股·40+截面）**
> | N | 每笔超额 | 胜率 | IC | 超额/每天占用资金 | 独立窗口≈ | 3日→N日爆雷率(高动量) |
> |---|---|---|---|---|---|---|
> | 2 | +0.77% | 59% | 0.089 | **+0.39%/天** | 22 | — |
> | **3(推荐)** | +0.92% | 55% | 0.096 | **+0.31%/天** | 15 | 21% |
> | 5 | +0.89% | 55% | **0.064(全场最低)** | +0.18%/天 | 8 | 31% |
> | 7 | +1.65% | 55% | 0.110 | +0.24%/天 | 6 | — |
> | 10 | +2.30% | 58% | 0.084 | +0.23%/天 | **仅4(不可靠)** | **43%** |
>
> **建议**：①用户未指定时仍按契约默认 **N=5**；若用户重视资金周转效率，可明确建议改用 **N=3**，复述口径后再传给引擎——N=3 是资金周转效率最高档里统计最扎实的选择，但不是静默默认值；②每笔超额随 N 变大是真的，但 N=7/10 的截面重叠严重（10日仅≈4个独立窗口），数字底座薄；③**爆雷率随 N 单调升**（持有越久撞崩盘日概率越高），"拿久点更稳/能涨回来"已被否决（崩后 84-89% 先触-8%止损，真·等回来仅 9-11%）——想吃长动量用 **P14-3 赢家延展**（浮盈才延，输家快刀），不是拉长 N；④ N=5 虽是兼容默认值，但处于权重预设低谷（既非超短配比也非波段配比），必须向用户披露这一取舍。
>
> **⚠ 爆雷率引用规范（对用户报数必须带口径，防止新会话裸引数字）**：爆雷率=3日窗内曾触-8%。三个不同口径不可混用——
> ① **21%** = 无过滤高动量票基线（面板画像，不是现行管线的值）；② **42.9%** = P13/P14 改造前实盘终排前2名的真实爆雷率（17期14笔，DOC§0）；③ **1.7%** = P14稳健线设计目标，**引用时必须带全后缀：`1.7%·95%CI 0.6–4.9%·30笔滚动复验中·0合格期=空仓`**，禁止说成"<3%已证明/系统爆雷率1.7%"。现行管线的诚实表述：可买票经 P13-1+P14 过滤，稳健线合格票按①→③的画像收敛，但**改后实盘累计样本尚不足，复验期内一律引用带CI的说法**。
...
### 复核模式（买入日 T+1 早上 9:15–9:25，**必做；尤其有过热/试探/催化已兑现票**）
...
```
复核只认同一份已验收 JSON 中持久化的 `stop`。任一候选缺 `stop` 就停止复核并重跑选股/裁定；不得使用
`recheck.py` 为旧工件保留的 `close-1.3×ATR` 兼容回退生成新止损。
...
> 一句话总结给用户：**在 Codex 新对话输入 `/skills` 选择本 skill，或直接 `$weekly-ashare-rank 天数=5`；backtest+策略验证按需自动跑；每次跨源校验价格、给出排名 edge 裁定；recheck 是买入日早上的必做环节，持有后说“复盘”自动算战绩。**
...
- **（2026-07-07 新增·P11 资金流×事件面×兑现风险带，引擎已自动执行——高能环境 T前5日主力净流出1.48亿/仅1天为正、07-06 买入日跌停的教训）**：
  ...
  5. **U2 高位长上影·软gate + U3 弱信号叠加升级（2026-07-07 陷阱研究校准）**：针对"竞价正常、盘中崩"的高能式陷阱（历史头部选择中占比~5%，recheck 拦不住）。面板6152股票日回测：**T日上影≥3%且高位 → 次日盘中崩概率4.11%=基准3.4倍（最强单一前兆）** → U2软gate=仓位减半+排名−6+高开>2%放弃，防守/观望升级剔除。**U3铁律：弱信号（兑现带/长上影/剧烈换手[高位大阴后大阳]/M6主力流出/F10事件负面≤−6）任意≥2项叠加 → 直接剔除**（`weak_signals`/`weak_n`/`risk_stack_out` 字段；高能07-06是五重叠加）。回测评定：历史41个头部选择的6个坏案例 5个剔除+1个半仓、组合买入日期望收益从−0.33%转+0.16%；面板层面政策成本仅−0.06%/日均收、换来陷阱暴露−24%且完全避开"次日≤−5%概率19%"的最差组。**注意单一弱信号触发后~96%的情况次日并不崩——单项只降档不剔除，防止误伤赢家（数据上一刀切会砍掉29/35好案例）**。
  6. **U4 环境分临界降档**：环境分踩在档位下边界±3分内（40-43/55-58/70-73）→ **按低一档执行**（07-03环境分正好40.0按防守出了5只可买、次日高能跌停的教训——风险闸门的边界误差向下取整）。reasons 里会写"踩临界带按低一档执行"。
  7. **U1 条件单硬纪律（执行层，防陷阱的唯一确定性手段）**：陷阱型=竞价看不出、盘中没人看盘就崩。报告「操作策略」块必须包含一条：**买入成交后立即挂券商条件单（跌破买区下沿或−4%自动止损）**——高能案例里这条能把−10%截成−4~5%，比任何选股信号都可靠。
...
- **（2026-07-14 新增·P14 稳健两票模式，引擎已自动执行——爆雷率研究）**：非进攻档，引擎给每只算 `steady_ok` **稳健线**（`ATR≤4% + 5日涨幅≤10% + 量比≤2 + 60日位≤85 + 0弱信号`），并把「稳健线合格 ∧ 有≤7天新鲜催化 ∧ 可买」中 rank_score 最高的**两只（强制不同 `industry`）物理置顶为终排第1/2**（`p14_pick=1/2`，HTML 深绿🛡徽标；仅合格未入选=浅绿🛡徽标）——**前两名天然=全场最不易爆雷的两只**。
  依据（220只×187日=40509股票日面板）：稳健线单只3日爆雷率(触-8%) **1.7%**（3/174，95%CI 0.6–4.9%）vs 高动量基线 21.0%；同日两只至少一雷 2.1%、双雷样本内 0；ATR 是最强单一杠杆（≤5% 9.0% vs >7.4% 29.9%），其余"有效filter"（振幅/乖离/换手/5日涨幅/60位）与 ATR 同维度、叠加增益<1pp。
...
`策略验证：<verdict>（Top-K超额 +x% · 胜率 y% · 排名IC z · 有效截面 n · 验证窗口市况 <val_market_regime>）`。

**P13-4/5 市况口径披露（2026-07-13，防"顺风IC外推"）**：validate 现输出 `val_market_regime`（按样本市场平均：>+1%上涨段 / <-1%下跌段 / 其余震荡段）与 `val_market_regime_note`，报告顶部策略验证行**必须带出市况**。**若验证窗口=上涨段而当期环境分<55**：①在"真实选股力"行明写"⚠ 该edge测自上涨截面，未在当前退潮市况验证，动量因子退潮期会反向"；②当期考虑 `--weights none` 或人工把动量桶权重减半后重跑（说明理由）；③置信度在 P1 的 IC 封顶之上再降一档。（教训 07-13：IC=0.094 测自市场平均+2.32%的上涨截面，被默认用在创业板-4.4%后的退潮日，头部票当日-6%。）
...
1. **跨源价格**：看引擎 JSON 里每只的 `verify.status`。引擎现已做**四源共识校验（东财 / 腾讯 / 新浪 / 雅虎）**：东财限流时自动用 `腾讯×新浪` 交叉验证、两国内源不足时外网雅虎(沪市)补位，状态会写成 `一致(腾讯×新浪)` 这种**带参与源**的形式。处置：
   ...
   - **P3-cache 脏缓存硬门禁**：对≥2源的一致价格取中位数，与该候选用于因子/价位计算的引擎 `close` 比较；任一票偏差 **>0.5%**，即判定当天完整日K缓存未落盘或已污染，停止裁定与出报告，整次选股加 `--refresh` 重跑。不得只改 JSON 的 `close` 或买入价绕过；`validate-weekly-engine` 会机械拒绝并提示 `--refresh`。
...
> 📐 **策略验证**：<通过/弱通过/未通过>（Top-K超额 +x% · 胜率 y% · 排名IC z · 有效截面 n · **验证窗口市况 <上涨/震荡/下跌段>**）— 引擎走样本回测，置信度上限据此设定；市况=上涨段而当期环境<55 → 必须明示"edge未在退潮市验证"（P13-4/5）
...
> **P4：若 Agent⓪=观望(<40)，本块不出下面这张8只表**，改为：`**本期结论：空仓观望**` + 「观察池」列全部候选代码+一句原因（暂不入场）。**P13-3：不点名观察标的、不给任何价位**（旧"至多1只观察标的+试探仓位"条款已废止，07-13中电港教训）。仅在 进攻/中性/防守 档才出下表。
...
> 并给每只打 `entry_status`（`可买`/`观察·不入场`/`双门槛未过·观察(P13-1)`/`过热·默认剔除(观察)`/`一字板·难买入`/`价格存疑·待核`）。
> HTML 顶部自动出**🌡 环境横幅**（regime/环境分/总仓上限/预案），每张卡自动出**状态徽标**（可买=绿、其余=灰且卡片灰化），不可买的票"买入"自动改"参考价"、目标/补仓清"—"。
> **P13-3（2026-07-13）：观望档 render 自动隐藏每张卡的全部操作价位**（买入/目标/止损/补仓/入场/放弃整块换成"—不给价位"），且**禁止把任何票的 entry_status 改成"观察标的"类点名**（旧"唯一观察标的"做法已废止）。**P13-5**：`validation` 里带 `val_market_regime` 时 HTML 验证行自动显示"验证窗口市况"。
...
3. **A股特性与个人可交易池**：T+1（次日才能卖）、涨跌停、流动性(日均额>1亿)、剔除 ST/次新；候选代码只允许 `00*`/`30*`/`60*`，明确排除科创板 `68*`（含688/689）与北交所 `8*`/`4*`。这是账户可买性合同，不得在重构 universe 时静默放开；`validate-weekly-engine` 会机械拒绝越界代码。
...
日常选股不得执行研究脚本。用户要求重校准、追溯规则来源或评估新 gate 时，先完整阅读
`references/RESEARCH_LEDGER.md`，再从 `scripts/research/legacy_cc/` 选择对应实验；必须保留样本窗、偏差、
走样本口径与“采纳/否决”状态，未经复验不得把研究结果写回生产规则。

完整运行说明见同目录 `DOC.md`；研究 provenance 见 `references/RESEARCH_LEDGER.md`。
