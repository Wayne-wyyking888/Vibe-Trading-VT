...
- **三条硬规则（2026-06-08 周一实战教训，不可跳过）**：
  1. **过热上限**：`5日涨幅>20% 且 60日位>90`（或 乖离MA10>12%）的票，**综合排名不得进前2**、
     入场只许"深回调低吸"、仓位上限减半。（教训：博威合金量化分85.6排第1，5日+25.6%/60位91/乖离+14%，
     周一买入周二即止损-2.2%，是当期唯一止损单；而它的过热信号选股当晚全都看得见。）
...
1. **跨源价格**：看引擎 JSON 里每只的 `verify.status`。`存疑(跨源偏差大)` → 该票买入价/止损不可信，先 WebSearch/再拉一次行情人工核对，对不上就**剔除**；`单源未校验`（东财被限流时）→ **P3：单源一律不得标「✓已验证」，最高只能标「⚠单源(价格未交叉验证)」**。若全表都是单源（东财整体限流），先用腾讯K线脚本对 Top 几只人工再拉一次双源核对；核不上的不进买入方案。
...
> **P3 强约束**：`✓已验证` 只发给**同时满足**「价格跨源一致 + 数值自洽(止损<买入<目标且R:R≥2) + 催化剂可追溯到≤7天WebSearch出处」的票；只要价格是单源/缓存，标记里**必含「⚠单源」**，绝不能出现"✓已验证(价格仅单源)"这种自相矛盾写法。
