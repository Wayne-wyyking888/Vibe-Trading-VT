...
edge 集中在**防守日**（信号96%来自防守期，防守≥9天是全研究最强格子82%胜/13.6%雷）。
...
1. **跑引擎**（PowerShell + 刷PATH + `dangerouslyDisableSandbox: true`，约2-4分钟拉480只K线）：
   ...
   （防守日·总分≥18 ∥ 非防守日·个股分≥15，均须 ATR≤4）→ 过线票执行方案 → 影子日志 →
...
3. **裁定同步进HTML（--adjudicate，2026-07-15 新增）**：把每只过线票的裁定写入
   ...
   `{"T":"引擎T日","rulings":{"代码":{"verdict":"✓|?|✗","why":"理由+日期来源"}}}`），然后跑：
   ```powershell
...
6. **影子复验期**：累计30笔了结前，报告顶部必须标"影子期：建议纸面跟踪或仓位减半"；30笔后 --review 实测
   与回测口径偏离>10pp → 回到研究台重校准。def_days/idx_rsv 为影子字段随每笔记录（防守≥9天/大盘RSV15-40
   是待复验强格子，未进规则）。
7. **已被数据否决、禁止再加**：资金流(叠加后反向)、多元学习权重(OOS全灭)、K线企稳形态(安慰剂)、
   "拿久等回来"(84-89%先触止损)、P13-2式已兑现降级。新想法一律先面板走样本再进规则。
