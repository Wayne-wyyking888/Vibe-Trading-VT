...
一致预期来源、指标预期与前值、公布后观察项完整可审计；CPI 必须用 `cpi_*` 专属 ID，PPI 必须用 `ppi_*`
专属 ID，分别包含总项/最终需求与核心口径的环比/同比四项，并逐项保存 `metric_definition`，禁止混用口径。
每个事件必须分别审计调查/市场定价、经济日历、机构/媒体预览三路；固定指标必须逐项记录采用值或无可靠估计。
